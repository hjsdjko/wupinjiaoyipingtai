    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(wupin !=null){
            ...
            BeanUtils.copyProperties( wupin , view );//把实体数据重构到view中
                ...
                //级联表
                ShangjiaEntity shangjia = shangjiaService.selectById(wupin.getShangjiaId());
                if(shangjia != null){
    ...
    */
    ...
    public R save(@RequestBody WupinEntity wupin, HttpServletRequest request){
        ...
        if(false)
            ...
            wupin.setShangjiaId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<WupinEntity> queryWrapper = new EntityWrapper<WupinEntity>()
            .eq("shangjia_id", wupin.getShangjiaId())
            .eq("wupin_name", wupin.getWupinName())
            .eq("wupin_types", wupin.getWupinTypes())
            .eq("wupin_kucun_number", wupin.getWupinKucunNumber())
            .eq("wupin_clicknum", wupin.getWupinClicknum())
            .eq("shangxia_types", wupin.getShangxiaTypes())
        ...
        if(wupinEntity==null){
            wupin.setWupinClicknum(1);
            wupin.setShangxiaTypes(1);
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            WupinEntity wupinEntity = new WupinEntity();
//                            wupinEntity.setShangjiaId(Integer.valueOf(data.get(0)));   //商家 要改的
...
//                            wupinEntity.setWupinOldMoney(data.get(0));                    //物品原价 要改的
//                            wupinEntity.setWupinNewMoney(data.get(0));                    //现价 要改的
//                            wupinEntity.setWupinClicknum(Integer.valueOf(data.get(0)));   //点击次数 要改的
//                            wupinEntity.setWupinContent("");//照片
//                            wupinEntity.setShangxiaTypes(Integer.valueOf(data.get(0)));   //是否上架 要改的
    ...
    public R detail(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        WupinEntity wupin = wupinService.selectById(id);
            if(wupin !=null){
                ...
                //点击数量加1
                wupin.setWupinClicknum(wupin.getWupinClicknum()+1);
                wupinService.updateById(wupin);
